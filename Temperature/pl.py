...
class PhonoBase:
    ...
    * get_forces()
    ...
    def plot_phonon_dispersion_bands(self, absv=False, tag='', writeFile=True, wave_vector_labels=True, customPath=None):
        """
        ...
        # customPath=[[0.,0.,0.],[1.,1.,0.]]
        bands_and_labels = self.get_path_using_seek_path(customPath=customPath)
        ...
        plt.xlabel('Wave vector'+' [Å]' if not wave_vector_labels else '')
        ...
        if writeFile:
            ...
            # print(pb_data)
            # print(len(curves))
            cpath = '_' + str(customPath).replace(' ', '') if customPath else ''
            with open(os.path.join(os.getcwd(), f'phonon_dispersion_bands{tag}{cpath}.csv'.replace('],[', '_').replace('[[', '').replace(']]', '')).replace(',', ''), 'w') as file:
                for i, curve in enumerate(curves):
                    # print(i,curve)
                    # if not nBands:
                    #     break
        ...
        plt.savefig("Cr_dispersion.png", dpi=300, bbox_inches='tight')
        plt.show()
